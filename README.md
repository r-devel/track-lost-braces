...
- [track_lost_braces.R](track_lost_braces.R) contains the code that identifies packages on CRAN with the `NOTE`, compares them with the latest version of the [tracking spreadsheet](https://docs.google.com/spreadsheets/d/1qL5s2okfQmh_ufwh3MS6rJPzIlLmJzIN2g9u2loFzkA), and updates accordingly. It also checks package source repositories via [check_repo_fixes.R](check_repo_fixes.R) to detect packages where the maintainer has resolved the issue on the default branch, setting their `PR_status` to `"Fixed by maintainer on repo"`. The script produces a `track_lost_braces` data frame combining CRAN information, PR details, and repo fix statuses.
...
- [check_repo_fixes.R](check_repo_fixes.R) provides helper functions to check the source repositories of packages with lost braces NOTES to identify if the Rd file issues have already been fixed in the default branch of the repository but not yet released to CRAN.
